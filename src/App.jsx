...
const savedProfile = JSON.parse(localStorage.getItem('bodycount-profile') || 'null')
...
function calculateTargets(profile) {
  if (!profile) return { calories: 2200, protein: 160, water: 8 }
  const base = 10 * (profile.weight * 0.453592) + 6.25 * (profile.height * 2.54) - 5 * profile.age + 5
  const active = base * Number(profile.activity)
  const calories = Math.round(profile.goal === 'cut' ? active - 450 : profile.goal === 'bulk' ? active + 300 : active)
  return { calories, protein: Math.round(profile.weight * (profile.goal === 'cut' ? 1 : 0.85)), water: 8 }
}

function App() {
  const [profile, setProfile] = useState(savedProfile || { name: 'Jose', age: 25, weight: 180, height: 70, goal: 'cut', activity: 1.45 })
  const [onboarded, setOnboarded] = useState(Boolean(savedProfile))
  ...
  useEffect(() => localStorage.setItem('bodycount-profile', JSON.stringify(profile)), [profile])
  ...
  function addWeight() { setWeightLog([...weightLog.slice(-6), Number(weightLog.at(-1) || profile.weight) - 0.5]) }
  function completeOnboarding(e) { e.preventDefault(); setOnboarded(true) }
  ...
  return (
    <main className={darkMode ? 'layout dark' : 'layout'}>
      {!onboarded && <section className="onboarding card"><p className="eyebrow">Personal setup</p><h1>Build your Bodycount plan.</h1><p className="status">Set your body data and Bodycount will calculate your daily calories and protein automatically.</p><form className="form" onSubmit={completeOnboarding}><div className="form-grid"><label>Name<input value={profile.name} onChange={(e)=>setProfile({...profile,name:e.target.value})}/></label><label>Age<input type="number" value={profile.age} onChange={(e)=>setProfile({...profile,age:Number(e.target.value)})}/></label><label>Weight lb<input type="number" value={profile.weight} onChange={(e)=>setProfile({...profile,weight:Number(e.target.value)})}/></label><label>Height inches<input type="number" value={profile.height} onChange={(e)=>setProfile({...profile,height:Number(e.target.value)})}/></label></div><div className="goal-row"><button type="button" onClick={()=>setProfile({...profile,goal:'cut'})}>Cut fat</button><button type="button" onClick={()=>setProfile({...profile,goal:'maintain'})}>Maintain</button><button type="button" onClick={()=>setProfile({...profile,goal:'bulk'})}>Build muscle</button></div><button type="submit">Start Bodycount</button></form></section>}
      {onboarded && <><aside className="sidebar"><div className="logo">Bodycount<span>AI</span></div><a href="#overview">Overview</a><a href="#scan">Scan</a><a href="#review">Review</a><a href="#history">Meals</a><a href="#insights">Insights</a><button className="mode" onClick={() => setDarkMode(!darkMode)}>{darkMode ? 'Light' : 'Dark'}</button></aside>
      <section className="content">
        <header className="hero" id="overview"><div className="hero-copy"><p className="eyebrow">AI calorie tracker</p><h1>{profile.name}, your plan is live.</h1><p>Goal: {profile.goal}. Bodycount calculated your targets automatically from your profile.</p><div className="hero-actions"><button onClick={() => setScannerOpen(true)}>Open scanner</button><a href="#review">Manual entry</a><button onClick={() => setOnboarded(false)}>Edit profile</button></div></div><div className="today-card"><span>Today</span><strong>{totals.calories}</strong><small>of {targets.calories} calories</small><div className="circle" style={{ '--p': `${caloriePercent * 3.6}deg` }}><b>{Math.round(caloriePercent)}%</b></div></div></header>
      ...
      </section><nav className="bottom-tabs"><a href="#overview">Home</a><a href="#scan">Scan</a><a href="#review">Review</a><a href="#history">Meals</a></nav>{scannerOpen && <section className="scanner-modal"><div className="scanner-sheet"><button className="close" onClick={() => setScannerOpen(false)}>Close</button><h2>Fullscreen scanner</h2><p>Upload a photo or use demo AI scan.</p><label className="upload"><input type="file" accept="image/*" capture="environment" onChange={uploadPhoto} />📸 Choose photo</label><button onClick={analyze}>Run demo scan</button></div></section>}</>}
    </main>
  )
}
export default App
