  ...
  return (
    <main className={darkMode ? 'layout dark' : 'layout'}>
      ...
      <section className="content">
        ...
        <section className="insight-grid" id="insights"><div className="card coach"><h2>Coach</h2><p>{coach}</p><div className="chips"><span>Protein first</span><span>Water before snacks</span><span>Lean dinner</span></div></div><div className="card"><h2>Weekly rhythm</h2><div className="mini-chart">{[62,78,42,88,70,54,92].map((h, i) => <span key={i} style={{ height: `${h}px` }}><b>{i + 1}</b></span>)}</div><p className="status">Visual progress preview.</p></div><div className="card"><h2>Hydration</h2><p className="status">Track water like a real wellness app.</p><div className="water-row">{Array.from({ length: targets.water }).map((_, i) => <button key={i} className={i < water ? 'water active' : 'water'} onClick={() => setWater(i + 1)}>💧</button>)}</div></div></section>
